...
colA[:,-1] = np.linspace(0.5, 1, cmap_lip.N) #replaces the last column (alpha channel) in colA with values from 0.75 to 1, creating a gradient in transparency across the color map.

sm_alpha = ListedColormap(colA)

cmap_oslo=cm.oslo_r
colA_oslo = cmap_oslo(np.arange(cmap_oslo.N))
colA_oslo[:,-1] = np.linspace(0.5, 1, cmap_oslo.N) #replaces the last column (alpha channel) in colA with values from 0.75 to 1, creating a gradient in transparency across the color map.

sm_alpha_oslo = ListedColormap(colA_oslo)
...
ax3=fig.add_axes([0.65, .78, .15, 0.025]) # color bar
...
cbar = plt.colorbar(sm,cax=ax4,orientation='horizontal', extend='max')
