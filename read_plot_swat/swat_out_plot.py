...
rps = model.get_ray_paths(source_depth_in_km=evt_z, distance_in_degree=delta_sr_deg, phase_list=["P"])
rp = rps[0]
...
fig.add_trace(go.Scatter3d(
    ...
    marker=dict(
        ...
        color=dbaz,
        colorscale="RdBu",
        colorbar=dict(title="scatbaz - baz_GCP (deg)", x=-0.12, xanchor="left", len=0.75),
        opacity=0.7,
    ),
    name="Color: dbaz",
    text=hover,
...
fig.add_trace(go.Scatter3d(
    ...
    marker=dict(
        ...
        color=pval,
        colorscale="Viridis",
        colorbar=dict(title="sta_scat_p", x=-0.12, xanchor="left", len=0.75),
        opacity=0.7,
    ),
...
fig.update_layout(
    title="Scatterers in 3D (toggle color variable)",
    scene=dict(
