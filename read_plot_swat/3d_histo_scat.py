...
mask_P = df["evt_scat_phase"].isin(P_only) & df["sta_scat_phase"].isin(P_only)
...
lat = df_use["scatlat"].to_numpy()
lon = df_use["scatlon"].to_numpy()
dep = df_use["scatdepth"].to_numpy()
...
## bit to keep scaterers within a lat long window
...
samples = np.column_stack([lat, lon, dep])
...
#### CHOOSE DEPTH HERE..
z_target = 550.0
# for z_target in np.arange(50,1350,200):
for z_target in [250]:
    ...
    cb.set_label("Raw scatterer count per 2°×2°×200 km bin")

    ax.set_title(f"Scatterer density at ~{z_target:.0f} km")
    plt.savefig("figs_samples_depth/230402_180411_{}km_rest.png".format(z_target), dpi=300, bbox_inches='tight', pad_inches=0.1)
    plt.close()
