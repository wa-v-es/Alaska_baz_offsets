...
def plot_baz_circle(m,file,marker):
    for line in open(file,'r'):
        line=line.split()
        if float(line[2])-1.5*float(line[3]) < float(line[1]) < float(line[2])+1.5*float(line[3]) and float(line[3]) < 3:
...
#
folder_pattern_pa = "/Users/keyser/Research/AK_all_stations/sac_files/max_vals_coherence/*maxVals_*_slow.txt"
folder_pattern_sa = "/Users/keyser/Research/AK_all_stations/sac_files/max_vals_coherence/SA/*maxVals_*_slow.txt"
...
plt.savefig('eq_all_PA_baz_offset_all_slow_new.png',dpi=400,bbox_inches='tight', pad_inches=0.15)
